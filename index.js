...
// Get active usernames endpoint (NEW)
app.get('/api/active-usernames', (req, res) => {
    try {
        const activeUsers = {};
        Array.from(userSessions.entries()).forEach(([sessionId, username]) => {
            activeUsers[sessionId] = username;
        });
        res.status(200).json({
            status: 200,
            data: activeUsers
...
// Get CUMULATIVE total shares endpoint (NEW - FIXED)
app.get('/api/cumulative-total', (req, res) => {
    try {
        let cumulativeTotal = 0;
        
        // Add from ACTIVE processes
        Array.from(total.values()).forEach(process => {
            cumulativeTotal += process.count;
        });
        ...
        res.status(200).json({
            ...
            data: {
                ...
                activeCount: Array.from(total.values()).length,
                historicalCount: shareHistory.length
            }
        });
    } catch (error) {
        res.status(500).json({
...
// Get historical statistics endpoint (NEW)
app.get('/api/historical-stats', (req, res) => {
    try {
        ...
        res.status(200).json({
            ...
            data: {
                completedProcesses,
                stoppedProcesses,
                totalHistoricalProcesses: shareHistory.length
            }
        });
    } catch (error) {
        res.status(500).json({
      ...
      if (sharedCount >= amount) {
        ...
        if (existingIndex > -1) {
            ...
            shareHistory[existingIndex] = {
                ...
                url: url,
                username: userSessions.get(sessionId) || username,
                count: amount,
                target: amount,
                timestamp: new Date().toISOString(),
                completed: true
...
app.listen(PORT, () => {
  ...
  console.log('\x1b[36m%s\x1b[0m', '║                                                          ║');
  console.log('\x1b[36m%s\x1b[0m', '╠══════════════════════════════════════════════════════════╣');
  console.log('\x1b[32m%s\x1b[0m', `║  Spam Share server running on port ${PORT}                ║`);
  console.log('\x1b[35m%s\x1b[0m', '║  Developer: Kellan Kaya                                   ║');
  console.log('\x1b[31m%s\x1b[0m', '║  Access Keys: [HIDDEN]                                    ║');
  console.log('\x1b[34m%s\x1b[0m', `║  History file: ${HISTORY_FILE}                            ║`);
  console.log('\x1b[36m%s\x1b[0m', '║  Cumulative total shares enabled                          ║');
  console.log('\x1b[36m%s\x1b[0m', '║                                                          ║');
  console.log('\x1b[36m%s\x1b[0m', '╚══════════════════════════════════════════════════════════╝');
  console.log('');
  console.log('\x1b[33m%s\x1b[0m', '╔══════════════════════════════════════════════════════════╗');
  console.log('\x1b[33m%s\x1b[0m', '║                     SYSTEM INFORMATION                   ║');
  console.log('\x1b[33m%s\x1b[0m', '╠══════════════════════════════════════════════════════════╣');
  console.log('\x1b[36m%s\x1b[0m', `║  Server URL: http://localhost:${PORT}                    ║`);
  console.log('\x1b[36m%s\x1b[0m', '║  API Endpoints:                                          ║');
  console.log('\x1b[36m%s\x1b[0m', '║    • GET  /total - Active processes                      ║');
  console.log('\x1b[36m%s\x1b[0m', '║    • GET  /api/history - Share history                   ║');
  console.log('\x1b[36m%s\x1b[0m', '║    • GET  /api/cumulative-total - Cumulative shares      ║');
  console.log('\x1b[36m%s\x1b[0m', '║    • GET  /api/historical-stats - History stats          ║');
  console.log('\x1b[36m%s\x1b[0m', '║    • GET  /api/active-usernames - Active usernames       ║');
  console.log('\x1b[36m%s\x1b[0m', '║    • POST /api/submit - Start new share process          ║');
  console.log('\x1b[36m%s\x1b[0m', '║    • POST /api/stop/:id - Stop process                   ║');
  console.log('\x1b[36m%s\x1b[0m', '║                                                          ║');
  console.log('\x1b[33m%s\x1b[0m', '╚══════════════════════════════════════════════════════════╝');
  console.log('');
});