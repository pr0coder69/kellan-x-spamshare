...
// Get real-time totals endpoint (NEW)
app.get('/api/realtime-totals', (req, res) => {
    try {
        let activeTotalShares = 0;
        let activeProcesses = 0;
        
        // Calculate from active processes
        Array.from(total.values()).forEach(process => {
            activeTotalShares += process.count;
            activeProcesses++;
        });
        
        // Calculate from history
        let historicalTotalShares = 0;
        let completedProcesses = 0;
        let stoppedProcesses = 0;
        
        shareHistory.forEach(item => {
            historicalTotalShares += item.count;
            if (item.completed) {
                completedProcesses++;
            } else {
                stoppedProcesses++;
        ...
        // Combined total (active + historical)
        const combinedTotalShares = activeTotalShares + historicalTotalShares;
        ...
        res.status(200).json({
            ...
            data: {
                ...
                completedProcesses,
                stoppedProcesses,
                totalProcesses: activeProcesses + shareHistory.length
            }
        });
    } catch (error) {
        res.status(500).json({
...
app.listen(PORT, () => {
  ...
  console.log('\x1b[36m%s\x1b[0m', '║  Real-time statistics enabled                             ║');
  console.log('\x1b[36m%s\x1b[0m', '║                                                          ║');
  console.log('\x1b[36m%s\x1b[0m', '╚══════════════════════════════════════════════════════════╝');
  console.log('');
  console.log('\x1b[33m%s\x1b[0m', '╔══════════════════════════════════════════════════════════╗');
  ...
  console.log('\x1b[36m%s\x1b[0m', '║    • GET  /api/realtime-totals - Live statistics         ║');
